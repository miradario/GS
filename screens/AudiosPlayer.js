...
import { checkFav, addFav, removeFav, checkDownload } from '../../modules/firebaseAPI';
...
export default class AudiosPlayer extends React.PureComponent<ScreenProps<>> {
  ...
  async _playRecording(online) {
    ...
    console.log(url);
    if (!this.props.navigation.getParam('music') && this.props.navigation.getParam('checkbkgmusic') == 'yes') {
      url = url + '_nosounds';
    }
    console.log(i18n.locale.toUpperCase());

    const aws =
      'https://tlexeurope.s3.eu-central-1.amazonaws.com/audios/Audio/' + i18n.locale.toUpperCase() + '/' + url + '.mp3';
    console.log(aws);
    ...
    const { sound } = await Audio.Sound.createAsync(source, {
      shouldPlay: false,
      isLooping: false,
    }).catch(() => this.alertErrorPlaying('Check your internet connection'));
  ...
  _onPlaybackStatusUpdate = (playbackStatus) => {
    if (playbackStatus.isLoaded && playbackStatus.isPlaying) {
      ...
      let totalTime = '--';
      if (!isNaN(playbackStatus.positionMillis / playbackStatus.durationMillis)) {
        audioProgress = playbackStatus.positionMillis / playbackStatus.durationMillis;
  ...
  handleFinish() {
    ...
    this.props.navigation.navigate('Finished', {
      ...
      category: this.props.navigation.getParam('category', 'mindfullness'),
  ...
  render(): React.Node {
    ...
    const { isFav, elapsedPercent, elapsedTime, totalDuration, totalTime, currentPosition } = this.state;
    ...
    return (
      <BaseContainer title="Meditate" navigation={this.props.navigation} noPadding>
        <BackArrow handleBackPress={() => this.handleBack()} />
        <View style={style.container}>
          <View style={style.imgWrapper}>
            <CachedImage source={{ uri: photo }} style={style.img} title={`${key}_cover`} resizeMode="cover" noWrap />
            <Text style={style.audioTitle}>{this.capitalize(this.props.navigation.state.params.ptitle)}</Text>
            <Text style={style.durationTitle}>{this.props.navigation.state.params.duration}</Text>
          ...
          {this.state.playingStatus !== 'LOADING' && (
            ...
            >
              <TouchableOpacity style={style.infoButton} onPress={() => this.handleFinish()}>
                <Text style={style.infoText}>{translate('Meditation.Complete')}</Text>
              </TouchableOpacity>
            </View>
          )}
        </View>
        <View style={style.seekBarWrapper}>
          <View style={{ flex: 1 }}>
            {this.state.playingStatus === 'PLAYING' || this.state.playingStatus === 'PAUSED' ? (
