...
export default class ListMeditate extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      connected: true,
      sections: {
        BreathPrivate: [{ icon: "ios-leaf", title: "Meditations" }],
        BreathPublic: [{ icon: "ios-rocket", title: "Tales" }],
        Mindfulness: [
          {
            icon: "ios-infinite",
            title: "Videos",
        ...
        Sleep: [{ icon: "md-moon", title: "Sleep" }],
        Walk: [{ icon: "ios-walk", title: "Adivces" }],
        Music: [
          {
        ...
        DesktopYoga: [{ icon: "md-body", title: "Sounds" }],
...
const NoInternet = () => (
  <View style={{ marginTop: 10, padding: 30, alignContent: "center" }}>
    <Text style={{ color: COLORS.gray, fontSize: 15, textAlign: "center" }}>
      No internet connection. You can still play contents in the downloads
      section.
    </Text>
  </View>
);
      ...
      <View>
        <View style={[styles.meditationThumb]}>
          ...
          >
            ...
            ></Image>
            {/*  <FavButton
              handleFavPress={() => this.onFavClick()}
